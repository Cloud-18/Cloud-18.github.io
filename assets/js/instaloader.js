...
window.mishaProcessResult = function(data) {
  for (x in data.data) {
    newstiles.push({
      date: getImgDate(data.data[x].created_time),
      imageloc: data.data[x].images.standard_resolution.url,
      link: data.data[x].link,
      desc: data.data[x].caption ? data.data[x].caption.text : "",
      height: data.data[x].images.standard_resolution.height / 1.43125
    });
  }
};
