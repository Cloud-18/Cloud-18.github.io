function createNewsTiles() {
    function cardtile(newstile, index) {
        ...
        return '<div class="card" data-aos="fade-up"><div class="card-header">' + newstile.date + '</div><div class="card-body"><h5 class="card-title"></h5>' + img + '<p class="card-text">' + descriptionSnippet + ' ... </p><button type="button" class="btn btn-outline-primary" data-toggle="modal" data-target=".readmore' + index + '">Read More</button></div></div>'
    }
    function modaltile(newstile, index) {
        return '<div class="modal fade readmore' + index + '" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true"><div class="modal-dialog modal-lg"><div class="modal-content"><div class="modal-header"><h5 class="modal-title" id="exampleModalLabel">' + newstile.date + '</h5><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button></div><img src="' + newstile.imageloc + '" width="100%"><div class="modal-body"><p>' + newstile.desc + '</p></div></div></div></div>'
    }
