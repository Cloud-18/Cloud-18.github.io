function createNewsTiles() {
    ...
    function modaltile(newstile, index) {
        return '<div class="modal fade readmore' + index + '" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true"><div class="modal-dialog modal-lg"><div class="modal-content"><div class="modal-header"><h5 class="modal-title" id="exampleModalLabel">' + newstile.date + 
        '</h5><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button></div><img src="' + newstile.imageloc + 
        '" width="100%"><div class="modal-body"><p id="instaIcon">' + newstile.desc + '</p><button onclick="goToPost(\'' + newstile.link + '\')" class="btn btn-outline-primary" id="instaIcon"><span class="font-weight-bold">View on Instagram &nbsp;</span><i id="instaIcon" class="fa fa-instagram fa-2x"></i></button></div></div></div></div>'
    }
